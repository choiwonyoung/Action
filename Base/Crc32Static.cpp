...
//***********************************************
DWORD CCrc32Static::StringCrc32( const char * pszString )
{
	assert( pszString );
	...
	try
	{
		while( *pszString != '\0' )
		{
			CalcCrc32((BYTE)*pszString, dwCrc32);
			pszString++;
		}
	}
	catch(...)
	{
		...
		MessageBoxA( NULL, pszString, "CRC 持失 拭君", MB_OK );
#endif
		return 0;
	}
...
DWORD CCrc32Static::WStringCrc32( const wchar_t * pwszString )
{
	assert( pwszString );
	...
	try
	{
		while( *pwszString != L'\0' )
		{
			CalcCrc32( (BYTE)*pwszString, dwCrc32 );
			CalcCrc32( (BYTE)*(((char*) pwszString) + 1), dwCrc32 );
...
#ifndef _GAME_RELEASE_
		MessageBoxW( NULL, pwszString, L"CRC 持失 拭君", MB_OK );
#endif
		return 0;
	}
...
DWORD CCrc32Static::FileCrc32Win32(LPCTSTR szFilename, DWORD &dwCrc32)
{
	...
	try
	{
		...
		else
		{
			...
			bool bSuccess = (TRUE == ReadFile(hFile, buffer, sizeof(buffer), &dwBytesRead, NULL)) ? true : false;
			while(bSuccess && dwBytesRead)
			{
				for(dwLoop = 0; dwLoop < dwBytesRead; dwLoop++)
					CalcCrc32(buffer[dwLoop], dwCrc32);
				bSuccess = (TRUE == ReadFile(hFile, buffer, sizeof(buffer), &dwBytesRead, NULL)) ? true : false;
			}
		}
	}
	catch(...)
...
DWORD CCrc32Static::FileCrc32Assembly(LPCTSTR szFilename, DWORD &dwCrc32)
{
	...
	try
	{
		...
		{
			// There is a bug in the Microsoft compilers where assembly
...
DWORD CCrc32Static::FileCrc32Assembly(char *pBuffer, DWORD dwSize, DWORD &dwCrc32)
{
	assert( pBuffer );
	...
	try
	{
		...
		{
			dwBytesRead = MAX_BUFFER_SIZE;
			dwCount += MAX_BUFFER_SIZE;
		}

		//buffer = (BYTE*)pBuffer;
		memcpy( buffer, pBuffer, sizeof( BYTE ) * dwBytesRead );
		...
		while(bSuccess && dwBytesRead)
		{
			...
			{
				if( ( dwSize - dwCount ) >= MAX_BUFFER_SIZE )
				{
					//	buffer = (BYTE*)( pBuffer + dwCount );
					dwBytesRead = MAX_BUFFER_SIZE;
					memcpy( buffer, pBuffer + dwCount, sizeof( BYTE ) * dwBytesRead );
					dwCount += MAX_BUFFER_SIZE;
				...
				{
					//buffer = (BYTE*)( pBuffer + dwCount );
					dwBytesRead = ( dwSize - dwCount );
					memcpy( buffer, pBuffer + dwCount, sizeof( BYTE ) * dwBytesRead );
					dwCount += ( dwSize - dwCount );
...
UINT64 CCrc32Static::StringCrc64(const char * pszString)
{
	DWORD dwCrc32 = CCrc32Static::StringCrc32(pszString);
	DWORD dwCompute = (DWORD)ComputeHashCode(pszString);

	UINT64 ddwCrc64 = dwCrc32;
	ddwCrc64 = (ddwCrc64 << 32) | dwCompute;
	return ddwCrc64;
}